...
<form method="post"> 
    <div class="a032_header_block">
        <div class="a032_header_title">Список параметров вариации</div>
...
function createNewBlock(preselectedValue = null, presetValue = '', isInitialBlock = false) {
    const wrapper = $('.a032_table_wrapper');
    const existingBlocks = wrapper.find('.a032_tbody').length;
    ...
    const newBlock = $(`
        <div class="a032_tbody" data-block-id="${blockId}">
            <table class="a032_table">
                <tr class="a032_tr_body">
                    <td class="a032_td a032_td_id">
                        <div class="a032_input_wrapper">
                            <select class="a032_input_field_select" name="spec_name_${blockId}">
                                ${optionsHtml}
                            </select>
                            <label class="a032_input_label" for="spec_name_${blockId}">Параметр</label>
                        </div>
                    </td>
                    <td class="a032_td a032_td_name">
                        <div class="a032_input_wrapper">
                            <input class="a032_input_field" type="text" name="value_data_${blockId}" 
                                   placeholder="Простой текст" value="${presetValue || ''}">
                            <label class="a032_input_label" for="value_data_${blockId}">Значение параметра</label>
                        </div>
                    </td>
                </tr>
            </table>
    ...
    if (preselectedValue) {
        newBlock.find('.a032_input_field_select').data('prev-value', preselectedValue);
    }

    // Обработчики событий
    newBlock.find('.a032_action_button').click(function(e) {
        e.preventDefault();
        const select = newBlock.find('.a032_input_field_select');
    ...
    newBlock.find('.a032_input_field_select').change(function() {
        const select = $(this);
        const selectedValue = select.val();
        ...
        if (selectedValue) {
            ...
            if (newIndex !== -1) {
                ...
                spec[newIndex][4] = newBlock.find('.a032_input_field').val();
            }
        }
    ...
    newBlock.find('.a032_input_field').on('input', function() {
        const select = newBlock.find('.a032_input_field_select');
        const selectedValue = select.val();
...
function updateAllSelects() {
    ...
    $('.a032_input_field_select').each(function() {
        const val = $(this).val();
...
// Кнопка добавления
$('.a032_add_button').click(function(e) {
    e.preventDefault();
    createNewBlock();
});
